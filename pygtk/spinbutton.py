...
class Cryptography:
    ...
    def Encryption_func(self,widget,entry,place):
        ...
        for m in arr:
            power = pow(m,self.xx)
            modulus = modulo(power,self.mul)
            asci= asci + hex(modulus)
        for m in arr:
            intm.append(modulo(pow(m,self.xx),self.mul))
        print intm
    ...
    def Decryption_func(self,widget,bob_text,show):
        ...
        for i in range(0,length): # Separate hex from stream of hex
            stri = ""
        ...
        for t in range(p,length):
            stri = stri + asci[t]
        byte.append(str(stri))
        print byte              # End of hex

        dint = []
        for i in byte:
            dint.append(int(i[2:-1],16))
        print dint
        # Applying algo to decrypt
        mtoint = []
        for m in dint:
            p = pow(m,self.d)
    ...
    def __init__(self):
        ...
        # Key generation process starting here
        frame = gtk.Frame("Cryprography:")
        ...
        #hbox.pack_start(expo_val_level,True,True,0)
        hbox.pack_start(spinner3,False,True,0)
        #expo_val_level.set_text("0")
        ...
        encrypt_frame = gtk.Frame("Encryption :")
        child_vbox.pack_start(encrypt_frame,True,True,0)
